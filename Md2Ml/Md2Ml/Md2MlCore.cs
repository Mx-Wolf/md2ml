  ...
  public class Md2MlEngine: IMd2Ml, IDisposable
  {
    ...
    public Paragraph CreateParagraph(ParaProperties properties)
    {
      ...
      if (properties.StyleName != null)
        paraProp.Append(new ParagraphStyleId() { Val = properties.StyleName });
      if (properties.Alignment != JustificationValues.Left)
        paraProp.Append(new Justification() { Val = properties.Alignment });
      Indentation ind = new Indentation();
      if (properties.FirstLineIndent == 0)
        ind.FirstLine = (properties.FirstLineIndent * 567).ToString("n0").Replace(",", "");
      if (properties.LeftIndent == 0)
        ind.FirstLine = (properties.LeftIndent * 567).ToString("n0").Replace(",", "");
      if (properties.RightIndent == 0)
        ind.FirstLine = (properties.RightIndent * 567).ToString("n0").Replace(",", "");
      var para = CreateParagraph();
      para.Append(paraProp);
      return para;
    }
    ...
    public void WriteText(Paragraph paragraph, string text, FontProperties fontProperties)
    {
      ...
      { rp.Append(new FontSize() { Val = fontProperties.FontSize }); }
      if (fontProperties.Bold)
        rp.Append(new Bold());
      if (fontProperties.Italic)
        rp.Append(new Italic());
      if (fontProperties.Underline != UnderlineValues.None)
        rp.Append(new Underline() { Val = fontProperties.Underline });
      if (fontProperties.Strikeout)
        rp.Append(new Strike());
      if (fontProperties.WriteAs != VerticalPositionValues.Baseline)
        rp.Append(new VerticalTextAlignment() { Val = fontProperties.WriteAs });
      if (fontProperties.UseThemeColor)
        rp.Append(new Color() { ThemeColor = fontProperties.ThemeColor });
      else if (fontProperties.Color != null)
        rp.Append(new Color() { Val = string.Format("#{0:X2}{1:X2}{2:X2}", fontProperties.Color.Value.R, fontProperties.Color.Value.G, fontProperties.Color.Value.B) });
      run.Append(rp);
    ...
    public void MarkdownNumberedList(List<string> bulletedItems, string paragraphStyle)
    {
      foreach (var item in bulletedItems)
      {
        ...
        MarkdownStringParser.FormatText(this, paragraph1, item, new FontProperties());
      }
    }
    ...
    public void MarkdownBulletedList(List<string> bulletedItems, string paragraphStyle)
    {
      foreach (var item in bulletedItems)
      {
        Paragraph paragraph1 = CreateParagraph(paragraphStyle);
